...
bool dynapseConfigSet(caerDeviceHandle cdh, int8_t modAddr, uint8_t paramAddr, uint32_t param) {
	...
	switch (modAddr) {
		...
		case DYNAPSE_CONFIG_POISSONSPIKEGEN:
			...
			break;
		case DYNAPSE_CONFIG_LOCALFILT:
			printf("Sent local filter\n");
			return (spiConfigSend(&state->usbState, DYNAPSE_CONFIG_LOCALFILT, paramAddr, param));
			break;
...
bool dynapseConfigGet(caerDeviceHandle cdh, int8_t modAddr, uint8_t paramAddr, uint32_t *param) {
	...
	switch (modAddr) {
		...
		case DYNAPSE_CONFIG_LOCALFILT:
			printf("Received local filter\n");
			return (spiConfigReceive(&state->usbState, DYNAPSE_CONFIG_LOCALFILT, paramAddr, param));
			break;		
